...
private val StorageKey = "scalajs-lexical-showcase-state"
...
def main(): Unit =
  ...
  wirePersistenceControls(editor)
  ...
  if !loadStateFromStorage(editor) then
    resetToShowcaseContent(editor)
...
private def wirePersistenceControls(editor: LexicalEditor): Unit =
  Option(dom.document.getElementById("save-state")).foreach: button =>
    button.asInstanceOf[dom.HTMLButtonElement].onclick = (_: dom.MouseEvent) =>
      saveStateToStorage(editor)
      ()

  Option(dom.document.getElementById("load-state")).foreach: button =>
    button.asInstanceOf[dom.HTMLButtonElement].onclick = (_: dom.MouseEvent) =>
      if !loadStateFromStorage(editor) then
        updateStorageStatus("localStorage: Nothing saved yet")
      ()

  Option(dom.document.getElementById("reset-state")).foreach: button =>
    button.asInstanceOf[dom.HTMLButtonElement].onclick = (_: dom.MouseEvent) =>
      dom.window.localStorage.removeItem(StorageKey)
      resetToShowcaseContent(editor)
      updateStorageStatus("localStorage: Reset to showcase content")
      ()

private def saveStateToStorage(editor: LexicalEditor): Unit =
  try
    val serialized = js.JSON.stringify(editor.getEditorState().toJSON())
    dom.window.localStorage.setItem(StorageKey, serialized)
    updateStorageStatus("localStorage: Saved current EditorState")
  catch
    case error: Throwable =>
      dom.console.error("Failed to save showcase state", error)
      updateStorageStatus("localStorage: Save failed")

private def loadStateFromStorage(editor: LexicalEditor): Boolean =
  try
    val serialized = dom.window.localStorage.getItem(StorageKey)
    Option(serialized).exists: value =>
      val parsedState = editor.parseEditorState(value)
      editor.setEditorState(parsedState, js.Dynamic.literal())
      updateStorageStatus("localStorage: Loaded saved EditorState")
      true
  catch
    case error: Throwable =>
      dom.console.error("Failed to load showcase state", error)
      updateStorageStatus("localStorage: Load failed")
      false

private def resetToShowcaseContent(editor: LexicalEditor): Unit =
  editor.update(() =>
    val root = Lexical.$getRoot()
    root.clear()

    showcaseParagraphs.foreach(text =>
...
private def updateStorageStatus(message: String): Unit =
  val status = dom.document.getElementById("storage-status")
  if status != null then
    status.textContent = message

private val showcaseParagraphs = List(
